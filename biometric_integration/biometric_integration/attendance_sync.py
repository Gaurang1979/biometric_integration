...
def _as_local_naive(event_dt, timezone_name):
    return (
        event_dt.astimezone(ZoneInfo(timezone_name))
        .replace(tzinfo=None)
    )
...
def _stable_session_key(employee, log_date, first_event_key):
    digest = sha1(
        f"{employee}|{log_date}|{first_event_key}".encode()
    ).hexdigest()[:24]
...
def _delete_checkin(checkin_name):
    if not checkin_name:
        return
    try:
        frappe.delete_doc(
        ...
        frappe.log_error(
            ...
            f"Unable to remove obsolete biometric checkin {checkin_name}",
        )


def _create_or_update_checkin(
    employee,
    log_date,
    row,
    log_type,
    ...
    if not meta.has_field("biometric_session_key"):
        raise RuntimeError(
            "Employee Checkin biometric_session_key field is missing. Run bench migrate."
        )
    ...
    if existing:
        ...
        changed = False
        target_time = row.event_time
        if checkin.time != target_time:
            checkin.time = target_time
            changed = True
    ...
    if meta.has_field("latitude") or meta.has_field("longitude"):
        # The movement entry does not need coordinates; Employee Checkin keeps
        # the device coordinates when the custom fields exist.
        ...
        if coords:
            if meta.has_field("latitude"):
                checkin.latitude = coords.latitude
            if meta.has_field("longitude"):
                checkin.longitude = coords.longitude
...
def _reconcile_daily_log(log):
    ...
    for session in sessions:
        ...
        first_entry = session["rows"][0]
        last_entry = session["rows"][-1]
        ...
        for row in session["rows"]:
            checkin_name = None
            if row.name == first_entry.name:
                checkin_name = in_checkin
            elif row.name == last_entry.name and out_checkin:
...
def _normalize_device_events(device, raw_events, duplicate_seconds):
    ...
    for raw in raw_events:
        if int(raw.get("major") or 0) != 5:
            continue
        if int(raw.get("minor") or 0) != 75:
            continue
        ...
        employee = frappe.db.get_value(
            "Employee",
            {
                "attendance_device_id": employee_no,
                "status": "Active",
            },
            "name",
        )
...
def _default_scheduler_range(settings):
    now = frappe.utils.now_datetime()
    minutes = max(int(settings.scheduler_window_minutes or 15), 1)
    start = now - timedelta(minutes=minutes)

    # Re-read the current day so the integration can correctly determine the
    # employee's device sessions. This is intentionally broader than the
    # scheduler window; duplicate event keys prevent repeated movement rows.
    ...
    return day_start, max(start, day_start.replace(tzinfo=None)), now
...
def sync_all_devices(
    ...
    if from_datetime is None or to_datetime is None:
        from_datetime = frappe.utils.now_datetime() - timedelta(
            minutes=max(int(settings.scheduler_window_minutes or 15), 1)
        )
        to_datetime = frappe.utils.now_datetime()

        # The scheduler needs the day's history to recognize location changes
        # and to keep the latest event as the current OUT for an open session.
        local_now = to_datetime.astimezone(ZoneInfo(DEFAULT_TIMEZONE))
        from_datetime = local_now.replace(
            hour=0,
            minute=0,
            second=0,
            microsecond=0,
    ...
    duplicate_seconds = max(
        int(settings.duplicate_seconds or DEFAULT_DUPLICATE_SECONDS),
        0,
    )
        ...
        try:
            ...
            frappe.db.set_value(
                ...
                device.name,
                {
                    ...
                    "last_sync_status": (
                        f"Fetched {len(raw_events)}; processed {len(normalized)} "
                        "successful employee events"
                    ),
                },
                update_modified=False,
            )
    ...
    for event in all_events:
        log_date = _day_from_event(
            event["event_dt"],
            next(
                (
                    d.timezone or DEFAULT_TIMEZONE
                    for d in devices
                    if (d.serial_number or d.device_id or d.ip) == event["device_serial"]
                ),
                DEFAULT_TIMEZONE,
            ),
        )
    ...
    # Reconcile every affected employee/day from the complete stored movement
    # history, not just the current scheduler window.
    ...
    return {
        ...
        **totals,
        "devices": results,
        "message": (
            f"Fetched {totals['fetched']} events; stored "
            f"{totals['movement_created']} new movement events; "
            f"reconciled {totals['sessions']} device sessions."
        ),
    }
