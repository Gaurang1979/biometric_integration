...
def _month_from_event(event_dt, timezone_name):
    local_date = event_dt.astimezone(ZoneInfo(timezone_name)).date()
...
def _stable_name(employee, month):
    digest = sha1(f"{employee}|{month}".encode()).hexdigest()[:16]
    return f"MOV-{digest}"


def _stable_session_key(employee, event_date, device_identity, first_event_key):
    digest = sha1(
        f"{employee}|{event_date}|{device_identity}|{first_event_key}".encode()
    ).hexdigest()[:24]
...
def _get_or_create_monthly_log(employee, month):
    name = _stable_name(employee, month)
    if frappe.db.exists(MOVEMENT_DOCTYPE, name):
        log = frappe.get_doc(MOVEMENT_DOCTYPE, name)
        if not log.month:
            log.month = month
...
def _append_event(log, device, event):
    ...
    event_time = _as_local_naive(event["event_dt"], timezone_name)
    row = log.append("movement_entries", {})
    row.event_date = event_time.date()
...
def _load_events_for_log(log):
    return frappe.get_all(
        ...
        fields=[
            "name", "parent", "event_date", "event_time", "device_name",
            "location", "device_serial_number", "employee_device_id",
            "authentication_mode", "authentication_result", "card_no",
            "direction", "event_key", "session_key", "employee_checkin",
        ],
...
def _row_date(row):
    ...
    return row["event_time"].date()


def _sessionize(rows):
    """Sessionize separately for each employee/calendar day and physical device.

    A new calendar day always starts a new session. A device change also starts a
    new session. Therefore Office -> Factory -> Office on one day creates three
    sessions, while an IN on the next day can never become the previous day's OUT.
    """
    sessions = []
    current = None
    ...
    for row in rows:
        ...
        if (
            ...
            or event_date != current["event_date"]
        ):
            current = {
...
def _get_device_for_row(row):
    ...
    for device in settings.devices or []:
        device_serial = (device.serial_number or device.device_id or device.ip or "").strip()
...
def sync_all_devices(from_datetime=None, to_datetime=None, require_scheduler=False):
    ...
    for log_name in affected:
        try:
            ...
            result = _reconcile_monthly_log(log)
...
def sync_device(device_name, from_datetime, to_datetime):
    ...
    try:
        ...
        for name in affected:
            result = _reconcile_monthly_log(frappe.get_doc(MOVEMENT_DOCTYPE, name))
            sessions += result["sessions"]
            checkins += result["checkins"]
        frappe.db.commit()
        return {"status": "success", "device": device.device_name or device.ip, "fetched": len(raw_events), "processed": len(normalized), "movement_created": created, "sessions": sessions, "checkins": checkins}
    except Exception as exc:
        frappe.db.rollback()
        frappe.log_error(frappe.get_traceback(), f"Hikvision single-device sync failed: {device_name}")
        return {"status": "error", "device": device.device_name or device.ip, "message": str(exc)}
